...
keywords:
  ['typescript', 'schema', 'validation', 'type', 'inference', 'scheme-validation', 'zod', 'yup', 'joi', 'superstruct']
tags:
  - typescript
  - scheme-validation
  - zod
...
- 모든 조건을 커버하였을 때, 요구 조건이 변경되어서 `Post` 인터페이스가 달라진다면 어떻게 해야하지?
- 모든 검증 로직을 위한 함수를 작성해야 되나? 각 조건에 대한 분기 처리는 또 다시 작성해야 하나?
...
예를 들어, parsing 을 통과하지 못하면 다른 로직을 실행할 수도, 기존 값을 수정할 수도 있을 것입니다.
...
하위 호환성을 위해 기존 인터페이스를 유지 시켜줘야 할 필요가 있는 경우 `preprocess` 는 유용하게 사용할 수 있습니다.
